...
class PacbioSampleSheetCompiler
  ...
  # Returns a flattened list of wells from the multiple plates within a run
  def wells
  ...
  def self.generate_headers
    self::COLUMN_CONFIG.keys.map(&:to_s) # convert the symbols (from freezing) to strings
  end
  ...
  def self.evaluate_column(args, column_lambda)
    column_lambda.call(args) || '' # convert nils to empty strings
  end

  # Return an Array of values in a row for the given args
  def self.generate_row(args)
    self::COLUMN_CONFIG.values.map { |column_lambda| evaluate_column(args, column_lambda) }
  end
  ...
  def generate
    CSV.generate do |csv|
      ...
      sorted_wells.each do |_well|
        row = self::COLUMN_CONFIG.values.map { |value| value.call(c) }
