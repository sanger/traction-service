...
module AuthProviders
  ...
  class OktaJwtProvider
    ...
    # @param jwks_uri [String] The URI to fetch the JWKS (JSON Web Key Set) from Okta
    ...
    # Decodes and verifies the JWT using the configured issuer, audience, and JWKS.
    #
    ...
    # @raise [SocketError, Errno::ECONNREFUSED, Net::OpenTimeout, Net::ReadTimeout] for network errors when fetching the JWKS
    # @raise [JSON::ParserError] if the HTTP response is not valid JSON
    # @raise [JWT::JWK::Set::KidNotFound, JWT::JWK::Set::InvalidJWKError] if the JWKS is invalid or missing required fields
