...
namespace :legacy_ont_data do
  ...
  task remove: :environment do
    # Deleting records older than a year is a bit crude but we know the timeframe
    Plate.where("plates.created_at < '#{1.year.ago}'").by_pipeline(:ont).each do |plate|
      puts "-> Deleting plate #{plate.barcode}"
      # We use delete instead of destroy as we want to be careful about mixed child records
