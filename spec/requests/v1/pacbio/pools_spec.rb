...
# TODO: change hard coded request ids to dynamic
RSpec.describe 'LibrariesController', type: :request, pacbio: true do
  ...
  context '#get' do
    ...
    it 'returns the correct attributes', aggregate_failures: true do
      get "#{v1_pacbio_pools_path}?include=requests", headers: json_api_headers
  ...
  context '#create' do
    context 'when creating a singleplex library' do
      context 'on success' do
        ...
        it 'associates the pool, request, library request and tag', aggregate_failures: true do
          post v1_pacbio_pools_path, params: body, headers: json_api_headers
      ...
      context 'on failure' do
        context 'when two library requests are invalid' do
          ...
          it 'cannot create a request library' do
            expect { post v1_pacbio_pools_path, params: body, headers: json_api_headers }.to_not change(Pacbio::RequestLibrary, :count)
          end
