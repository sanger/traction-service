...
RSpec.describe 'SamplesController', type: :request do
  ...
  context '#get' do
    ...
    it 'returns a list of samples' do
      ...
      expect(json['data'][0]["attributes"]["created_at"]).to eq(sample1.created_at.strftime("%Y/%m/%d %I:%M"))
      ...
      expect(json['data'][1]["attributes"]["created_at"]).to eq(sample2.created_at.strftime("%Y/%m/%d %I:%M"))
