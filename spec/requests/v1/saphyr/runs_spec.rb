...
RSpec.describe 'RunsController', type: :request do
  ...
  context '#get' do
    ...
    it 'returns the correct attributes' do
      ...
      expect(json['data'][0]["attributes"]["created_at"]).to eq(run1.created_at.strftime("%Y/%m/%d %H:%M"))
      ...
      expect(json['data'][1]["attributes"]["created_at"]).to eq(run2.created_at.strftime("%Y/%m/%d %H:%M"))
    end
